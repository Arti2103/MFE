...
```
$ curl http://localhost:NaN/placeholder.txt
...
```
$ curl http://localhost:NaN/unauthorized
$ # result -> true
```
...
```
$ curl http://localhost:NaN/authorized
...
```
$ # POST /auth/login
$ curl -X POST http://localhost:NaN/auth/login -d '{"username": "maria", "password": "123"}' -H "Content-Type: application/json"
